...
// ─── Supabase ────────────────────────────────────────────────
// Uses your existing project envs. If your client is exported
// from somewhere else (e.g. '@/lib/supabase'), swap the import.
...
export default function PaperTemplatePage() {
  ...
  const [toast, setToast] = useState(null);              // { kind: 'success'|'error'|'info', msg: string }
  ...
  // ─── Load existing paper if one exists ─────────────────────
  const loadExisting = useCallback(async () => {
    ...
    try {
      ...
      if (data) {
        ...
        // No paper for this combo yet — fresh draft
        setPaperId(null);
        setStatus('draft');
        setSubmittedAt(null);
      }
  ...
  // ─── Styles ────────────────────────────────────────────────
  const s = {
    page: { minHeight: '100vh', background: SOFT, fontFamily: '"Inter", system-ui, sans-serif', color: INK },
    topbar: {
    ...
    h1: { margin: 0, fontFamily: '"Playfair Display", Georgia, serif', fontSize: 20, fontWeight: 700 },
    ...
    sectionNumber: {
      ...
      display: 'grid', placeItems: 'center',
      fontFamily: '"Playfair Display", Georgia, serif', fontWeight: 700, fontSize: 14, flexShrink: 0,
    },
    sectionTitleInput: {
    ...
    summaryNum: { fontFamily: '"Playfair Display", Georgia, serif', fontSize: 24, fontWeight: 700, color: GOLD },
    ...
    paperWrap: {
      ...
      padding: '40px 48px', boxShadow: '0 0 0 1px rgba(0,0,0,.06), 0 30px 60px rgba(14,31,61,.12)',
      color: '#000', fontFamily: '"Times New Roman", Times, serif', fontSize: 14, lineHeight: 1.55,
    },
    paperHead: { textAlign: 'center', borderBottom: `3px double ${INK}`, paddingBottom: 12, marginBottom: 14 },
    paperSchool: {
      fontFamily: '"Playfair Display", Georgia, serif',
      fontSize: 22, fontWeight: 700, letterSpacing: 0.5, color: NAVY, margin: 0,
    },
    paperTest: { fontSize: 14, fontWeight: 600, marginTop: 4, fontStyle: 'italic' },
    metaRow: {
      display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6,
      fontSize: 13, marginTop: 10, paddingTop: 10, borderTop: `1px solid ${INK}`,
    },
    metaCell: { display: 'flex', gap: 6 },
    metaLabel: { fontWeight: 700 },
    studentRow: {
      display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: 14,
      fontSize: 13, margin: '14px 0', paddingBottom: 10, borderBottom: `1px solid ${INK}`,
    },
    studentField: { display: 'flex', alignItems: 'flex-end', gap: 6, borderBottom: `1px dotted ${INK}`, paddingBottom: 2 },
    versionStamp: {
      position: 'absolute', top: 30, right: 30, border: `2px solid ${GOLD}`, color: NAVY,
      padding: '4px 14px', fontFamily: '"Playfair Display", Georgia, serif',
      fontWeight: 700, fontSize: 14, transform: 'rotate(4deg)', background: '#fff', letterSpacing: 1,
    },
    pSection: { marginTop: 18 },
    pSectionTitle: {
      ...
      alignItems: 'baseline', borderBottom: `1px solid ${GOLD}`, paddingBottom: 4,
    },
    pSectionMarks: { fontSize: 12, color: '#444', fontWeight: 600 },
    pInstr: { fontStyle: 'italic', fontSize: 13, marginTop: 4, marginBottom: 10, color: '#444' },
    pQuestion: { display: 'flex', gap: 8, marginBottom: 8 },
    pQNum: { fontWeight: 700, minWidth: 24 },
    pAnsLine: { borderBottom: `1px solid #999`, height: 22 },
    footer: {
      textAlign: 'center', marginTop: 24, paddingTop: 12, borderTop: `1px solid ${INK}`,
      fontSize: 11, fontStyle: 'italic', color: '#666',
    },
  };

  // Helper: locked-aware input style
  const lockedInputStyle = isLocked ? { ...s.input, ...s.inputLocked } : s.input;

  // ─── Render printable paper ─────────────────────────────────
  const renderPaper = (versionKey) => {
    const ver = versions[versionKey];
    const total = computeTotal(ver.sections);
    return (
      <div className="paper-page" style={{ ...s.paperWrap, position: 'relative' }}>
        ...
        <div style={s.paperHead}>
          ...
          <div style={s.metaRow}>
            ...
            <div style={{ ...s.metaCell, justifyContent: 'center' }}><span style={s.metaLabel}>Subject:</span> {meta.subject}</div>
            <div style={{ ...s.metaCell, justifyContent: 'flex-end' }}><span style={s.metaLabel}>Time:</span> {meta.timeAllowed}</div>
          </div>
          <div style={{ ...s.metaRow, gridTemplateColumns: '1fr 1fr', borderTop: 'none', paddingTop: 0 }}>
            <div style={s.metaCell}><span style={s.metaLabel}>Total Marks:</span> {meta.totalMarks}</div>
            <div style={{ ...s.metaCell, justifyContent: 'flex-end' }}>
              <span style={s.metaLabel}>Obtained:</span>
              <span style={{ borderBottom: `1px solid #000`, minWidth: 50, display: 'inline-block' }}> </span>
              / {meta.totalMarks}
            </div>
          </div>
        </div>
        ...
        <div style={s.studentRow}>
          <div style={s.studentField}><span style={s.metaLabel}>Name:</span><span style={{ flex: 1 }} /></div>
          <div style={s.studentField}><span style={s.metaLabel}>Roll No:</span><span style={{ flex: 1 }} /></div>
          <div style={s.studentField}><span style={s.metaLabel}>Date:</span><span style={{ flex: 1 }} /></div>
        </div>
          ...
          return (
            <div key={sec.id} style={s.pSection} className="paper-section">
              <h2 style={s.pSectionTitle}>
                ...
                <span style={s.pSectionMarks}>({sec.questions.length} × {sec.marksPer} = {sMarks} marks)</span>
              </h2>
              ...
              {sec.questions.map((q, qi) => {
                  ...
                  return (
                    <div key={q.id} style={{ marginBottom: 10 }}>
                      <div style={s.pQuestion}>
                        ...
                        <span style={{ flex: 1 }}>{q.text || '____________________________________________'}</span>
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '4px 16px', paddingLeft: 32, fontSize: 13 }}>
                        {(q.options || ['', '', '', '']).map((opt, oi) => (
                          <div key={oi}>
                            <span style={{ fontWeight: 600 }}>({String.fromCharCode(97 + oi)})</span> {opt || '__________'}
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                if (sec.type === 'fillblank' || sec.type === 'truefalse' || sec.type === 'match') {
                  return (
                    <div key={q.id} style={s.pQuestion}>
                      <span style={s.pQNum}>{qi + 1}.</span>
                      <span style={{ flex: 1 }}>{q.text || '____________________________________________________________'}</span>
                    </div>
                  );
                }
                if (sec.type === 'short') {
                ...
                return (
                  <div key={q.id} style={{ marginBottom: 18 }}>
                    ...
                    <div style={{ paddingLeft: 32 }}>
                      {Array.from({ length: 5 }).map((_, k) => <div key={k} style={s.pAnsLine} />)}
                    </div>
                  </div>
                );
              })}
        ...
        <div style={s.footer}>
          Paper Total: {total} / {meta.totalMarks} &nbsp;•&nbsp; Best of Luck &nbsp;•&nbsp; {meta.subject} — {meta.className}
        </div>
      </div>
    );
  };
