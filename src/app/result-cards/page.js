...
export default function ResultCardsPage() {
  ...
  async function load() {
    if (!secret || !code || !examId) return;
    setLoading(true); setError(""); setData(null);
    try {
      const res = await fetch(
        `/api/award-list/result-cards?code=${encodeURIComponent(code)}&examId=${encodeURIComponent(examId)}&${authQ()}`
      );
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-8">
        ...
        <div className="flex flex-wrap gap-3 items-end mb-4">
          <div>
            ...
            <select className={inputCls} style={inputStyle} value={mode}
          ...
          <div>
            ...
            <input type="password" value={secret} onChange={(e) => setSecret(e.target.value)}
                   name="rc-secret" autoComplete="off" data-1p-ignore data-lpignore="true"
          ...
          <button onClick={load} disabled={!secret || !code || loading}
