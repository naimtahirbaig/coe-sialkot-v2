...
export default function Page() {
  ...
  useEffect(() => {
    async function load() {
      const { data: result } = await supabase.from('students').select('*').order('created_at', { ascending: false }).limit(50)
      setData(result || [])
      setLoading(false)
    }
    load()
  ...
  return (
    <PortalShell role="management" activeNav="students">
      <StatCard label="Total Records" value={data.length} color="#3b82f6" loading={loading} />
      <div style={{ marginTop: 20 }}>
        <Panel title="Student Overview" noPad>
