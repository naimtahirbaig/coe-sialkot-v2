...
export default function Page() {
  ...
  useEffect(() => {
    async function load() {
      const { data: result } = await supabase.from('fee_payments').select('*').order('created_at', { ascending: false }).limit(50)
      setData(result || [])
      setLoading(false)
    }
    load()
